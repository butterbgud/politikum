...
function ActionBoard({ G, ctx, moves, playerID, matchID }) {
  ...
  return (
    <div className="w-full min-h-screen bg-[url('/assets/ui/table_v2.webp')] bg-cover bg-center text-amber-100">
      ...
      {/* Opponents */}
      <div className="fixed top-20 z-[700] flex justify-start gap-6 pointer-events-auto pl-6" style={MOBILE ? { left: '-80px', right: '80px' } : (opponents.length === 1 ? { left: '50%', transform: 'translateX(-50%)' } : { left: '2rem', right: 'auto' })}>
        ...
        style={(() => {
          ...
          if (MOBILE) {
            return {
              ...
              transform: `translateX(-50%) translateY(calc(${mobileOppZoomPid ? dy + ' + 120vh' : dy} + ${mobileHandOpen ? '100px' : '0px'}))`,
            };
          }
                    ...
                    >
                      {it.kind === 'face' && (
                        <img
                          ...
                          className="w-full h-full object-cover"
                          draggable={false}
                          onClick={(e) => { e.stopPropagation(); try { setMobileMyZoomCard(it.card); } catch {} }}
                        />
                      )}
                    </div>
                  );
